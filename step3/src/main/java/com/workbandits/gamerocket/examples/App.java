...
public class App {
    ...
    public static void main(String[] args) {
        ...
        get(new FreeMarkerRoute("/attack") {
            ...
            public ModelAndView handle(Request request, Response response) {
                ...
                Result<Map<String, Object>> result = gateway.action().run("on-load-attack-page", actionRequest);
                if (!result.isSuccess()) {
                    Map<String, Object> error = new HashMap<String, Object>();
                    error.put("message", result.getErrorDescription());
                ...
                return modelAndView(result.getTarget(), "attack.ftl");
            }
        });
        
        get(new FreeMarkerRoute("/fight") {
            ...
            public ModelAndView handle(Request request, Response response) {
                ...
                Result<Map<String, Object>> result = gateway.action().run("fight", actionRequest);
                if (result.isSuccess()) {
                    if ("error".equals(result.getTarget().get("status"))) {
                        Map<String, Object> error = new HashMap<String, Object>();
                        error.put("message", ((Map<String, Object>) result.getTarget().get("data")).get("message"));
